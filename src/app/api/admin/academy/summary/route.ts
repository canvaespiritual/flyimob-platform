...
export async function GET(req: NextRequest) { await requireUser(); const f = filters(req); if (Number.isNaN(f.from.getTime()) || Number.isNaN(f.to.getTime()) || f.from >= f.to) return NextResponse.json({ error: "invalid_period" }, { status: 400 }); return NextResponse.json(await academyCentralSummary(f)); }
